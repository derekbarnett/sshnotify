...
SETTINGS = {
    "show_highlight"     : "on",
    #trigger on highlighted nick/word
    "show_priv_msg"      : "on",
    #trigger on private message
    "ignore"             : "",
    #comma seperated list of strings to ignore, e.g. if using bitlbee,
    #you might want to add '@root: jabber,@root: otr' to avoid spam on
    #startup
    "pm-urgency"         : "critical",
    #notification urgency for private messages. valid values are: low,
    #normal,critical
    "pm-image"           : "",
    #location of notification icon for private messages, optional
    "mention-urgency"    : "normal",
    #notification urgency for highlighted strings, valid values are:
    #low,normal,critical
    "mention-image"      : "",
    #location of notification icon for highlighted strings, optional
    "extra_commands"     : "",
    #extra commands to pass via ssh, appeneded after notify command.
    #note: you need to include && to execute the commands sequentially
    #for example, in order to have espeak announce that you have a new
    #message and then play an audio file:
    #/set plugins.var.python.sshnotify.extra_commands && espeak 'hey, you have a new message' && aplay whatever.wav'
    "addresses"          : "localhost",
    #comma delimited lists of addresses (and options) to use via ssh
    #default is localhost. if no address is specified, this script
    #won't do anything for you if you set addresses to "",
    #it will reset to localhost the next time it loads.
    #if you need to specify a different display, add DISPLAY=:1
    #(or the appropriate number) by default, sshnotify will use
    #DISPLAY=:0, so there is no need to specify that in your address string
    ...
    #will send a notification to 3 places, you@localhost, foot@bar, and
    #bar@foo on port 1234 on display 1
...
def get_notified(data, bufferp, uber_empty, tagsn, isdisplayed,
    ...
    #if message contains an ignored string, don't send the notification
    ilist = get_ignore()
    for i in ilist:
    ...
    if (weechat.buffer_get_string(bufferp, "localvar_type") == "private" and
        ...
        if bufferz == prefix:
            ...
            #escaping all special characters so that the message formatting
            #doesn't fail
            prefix = re.escape(prefix)
            #notification message
            message = re.sub("'", '"', message)
            message = re.escape(message)
            #setting the command which will be passed by ssh to push the
            #notification
            #note the DISPLAY variable is now accounted for in the ssh part
            #of the command rather than this location
          ...
          #fire when ready
            ...
            for a in alist:
                if a != '':
                    ...
                    com = "ssh -X " + a + " " + dispnum + " "
                    #add all the bits together and send the notification.
                    #time out if it can't connect in 5 seconds
        ...
        for a in alist:
            if a != '':
                #if display is set in the address, do not apply default
                #value defined above
                if re.search('DISPLAY\=', a):
                    dispnum = ""
...
def notifying(data, bufferz, args):
    ...
    weechat.prnt("", """
the command /sshnotify won't do much for you,
see /help sshnotify for info on how to use this plugin""")
    return weechat.WEECHAT_RC_OK
